...
export const showInstallInstruction = async () => {
  ...
  if (shell !== 'bash' && shell !== 'zsh')
    complainAndExit('Completion is only supported for "zsh" and "bash" shells. Detected: ' + shell);
  ...
  if (wantsWrite) {
    ...
    if (!fs.existsSync(shellRCFile)) complainAndExit('Config file does not exist: ' + shellRCFile);
    ...
    console.log(pc.green('Completion should work now in new opened or sourced shells.'));
  } else {
    console.log(`
To install auto-completion in ${pc.bold(pc.yellow(shell))} add to your ${pc.blue(shellRC)} the following:
