...
const _getRow = (
  ...
  `  ${label}${' '.repeat(3 + longestLabelLength - label.length)}${description}${alias ? ` (alias: ${alias})` : ''}`;

const _getRows = <K extends string, T extends { description: string; alias?: string } & { [key in K]: string }>(
  label: string,
  values: T[],
  key: K
): string => {
  ...
  const rows = values
    .map((v) => _getRow(v[key], { description: v.description, alias: v.alias }, longestLabelLength))
    .join('\n');
  return `\n${label}:\n${rows}\n`;
};
